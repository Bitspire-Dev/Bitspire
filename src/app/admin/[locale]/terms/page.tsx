...
export default async function AdminTermsPage({ params }: PageProps) {
  const { locale } = params;

  try {
    const result = await client.queries.pages({ relativePath: `${locale}/terms.mdx` });
    ...
    console.error("Admin terms page not found", error);
    notFound();
  }
}
