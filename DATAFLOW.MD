...
- **Prefix locale jest as-needed:** domyślny PL bez `/pl`, EN z `/en`.
...
- [ ] Dla niepoprawnego locale zwróć `notFound()` (bez redirectu na `/pl`).
...
- **Locale** jest częścią ścieżki tylko dla EN (PL bez prefiksu `/pl`).
